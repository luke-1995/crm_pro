...
class Pagination(object):
    def __init__(self, current_page, all_count, base_url, query_params, per_page=20, pager_page_count=11):
        ...
        max_page_num = math.ceil(all_count/per_page)
        try:
            self.current_page = int(current_page)
            if self.current_page <= 0 or self.current_page>max_page_num:
                raise Exception()
        except Exception as e:
            self.current_page = 1
        self.query_params = query_params # 可以通过get_query_params获取
